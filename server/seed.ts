...
// Admin credentials come from env vars. In production they are required (no insecure defaults).
...
if (isProduction && (!ADMIN_EMAIL || !ADMIN_PASSWORD) && (!OWNER_EMAIL|| !OWNER_PASSWORD)) {
    console.error(
        "Seeding failed: ADMIN_EMAIL and ADMIN_PASSWORD must be set when NODE_ENV=production."
    );
    process.exit(1);
}
...
const seedData = async () => {
    try {
        ...
        // Default admin password/email, overridable via env vars (required in production)
        const adminPassword = await bcrypt.hash(ADMIN_PASSWORD || "admin123", salt);
        const userPassword = await bcrypt.hash("user123", salt);
        const ownerPassword = await bcrypt.hash(OWNER_PASSWORD || "owner123", salt);
